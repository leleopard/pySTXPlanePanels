...
| DESIGN-14 | As a user, every color picker button in the designer shows the current color as a small swatch (not as the button's own background, which fights the OS theme and makes the hex/alpha readout hard to read), and any custom colors I pick in the "Custom colors" palette are still there the next time I open the app. | ✅ |
